...
impl JavaRandom {
	...
	pub fn new() -> JavaRandom {
		let millis = std::time::SystemTime::now().duration_since(std::time::SystemTime::UNIX_EPOCH).unwrap().as_millis();

		JavaRandom::with_seed(unsafe { std::mem::transmute(millis as u64) })
	}
	...
	fn next(&mut self, bits: i32) -> i32 {
		...
		(unsafe { std::mem::transmute::<u64, i64>(std::mem::transmute::<i64, u64>(self.seed.0) >> (48 - bits)) }) as i32
	}
